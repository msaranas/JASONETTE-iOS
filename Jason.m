...
@interface Jason(){
    ...
    DEMOMenuViewController *menuController;
...
- (void)start{
    ...
    menuController = [[DEMOMenuViewController alloc] initWithStyle:UITableViewStylePlain];
   ...
   /// UITableViewController *rightViewController = [UITableViewController new];
    ...
    tab.viewControllers = @[frostedViewController];//nav];
    tab.tabBar.hidden = YES;
    
    [tab setDelegate:self];
   // UINavigationController *navi = [[UINavigationController alloc] initWithRootViewController:tab] ;
   /* sideMenuController = [LGSideMenuController sideMenuControllerWithRootViewController:navigationController
                                                                  leftViewController:leftViewController
                                                                   rightViewController:rightViewController];
    sideMenuController.rootViewStatusBarHidden = NO;
                          ...
                          delayUntilAuthorized:YES  // This parameter is optional, defaults to NO if omitted
                                         block:^(CLLocation *currentLocation, INTULocationAccuracy achievedAccuracy, INTULocationStatus status) {
                                             if (status == INTULocationStatusSuccess) {
                                                 ...
                                                 self->tmpcurrentLocation=currentLocation;
                                                 ...
                                                 [_placePicker pickPlaceWithCallback:^(GMSPlace *place, NSError *error) {
                                                     ...
                                                     if (place != nil) {
                                                         ...
                                                         [[Globals sharedInstance].viewModel setValue:place.formattedAddress forKey:kv[@"bindable"]];
                                                     } else {
...
- (void)showleftmenu{
    ...
    /*    navigationController.menu.backgroundColor = backgroundColor;
         navigationController.menu.borderColor = backgroundColor;
         navigationController.menu.shadowRadius = 0.0;
         navigationController.menu.separatorHeight = 0.0;
         navigationController.menu.itemHeight = 50.0;
         navigationController.menu.animationDuration = 0.1;
     */
       // navigationController.menu.itemHeight=28.0;
       /// navigationController.menu.items=menu_item_array;
      //  [navigationController toggleMenu];// showFromNavigationController:navigationController];
    }
...
- (void)reload{
    ...
    if(VC.url){
       [self networkLoading:VC.loading with:nil];
        ...
        [manager.requestSerializer setCachePolicy: NSURLRequestReloadIgnoringLocalCacheData ];//NSURLRequestReturnCacheDataElseLoad];
        if(VC.fresh){
...
- (void)setupHeader: (NSDictionary *)nav{
    ...
    if(!left_menu || [left_menu count] == 0){
        ...
        if(left_menu[@"text"]){
           ...
           // [button addTarget:navigationController action:@selector(toggleMenu) forControlEvents:UIControlEventTouchUpInside];
            ...
            if(left_menu[@"image"]){
                ...
                [manager loadImageWithURL:[NSURL URLWithString:image_src]
         ...
         //   [btn addTarget:navigationController action:@selector(toggleMenu) forControlEvents:UIControlEventTouchUpInside];
...
- (void)leftMenu{
    ...
    NSDictionary *href = [[VC.menu valueForKey:@"left"] valueForKey:@"href"];
     NSArray *items  = [[VC.menu valueForKey:@"left"] valueForKey:@"menuitems"];
    ...
    if(items && items.count > 0){
       ...
       //  [navigationController toggleMenu];
        // showFromNavigationController:navigationController];
    }
    }*/
...
- (void)setTabBarItem:(UITabBarItem *)item withTab: (NSDictionary *)tab{
    ...
    if(image){
        ...
        [manager loadImageWithURL:[NSURL URLWithString:image]
                              options:0
                             progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                                 
                             } completed:^(UIImage * _Nullable i, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
...
- (void)goview:(UIViewController*)viewController
{
        ...
        if([view.lowercaseString isEqualToString:@"web"]){
            ...
            if([transition isEqualToString:@"modal"]){
              ...
              //  UINavigationController *navigationController = [self.storyboard instantiateViewControllerWithIdentifier:@"formSheetController"];
                MZFormSheetPresentationViewController *formSheetController = [[MZFormSheetPresentationViewController alloc] initWithContentViewController:newNav];
               // formSheetController.presentationController.contentViewSize = newNav.preferredContentSize;//CGSizeMake(250, [UIScreen mainScreen].bounds.size.height ); // or pass in UILayoutFittingCompressedSize to size automatically with auto-layout
                ...
                formSheetController.presentationController.frameConfigurationHandler = ^(UIView * __nonnull presentedView, CGRect currentFrame, BOOL isKeyboardVisible) {
                    if (UIInterfaceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
                        if(href[@"windowheight"] && href[@"windowwidth"])
                        {
                            NSString *wheight=href[@"windowheight"];
                            NSString *wwidth=href[@"windowwidth"];
                            return CGRectMake(CGRectGetMidX(weakFormSheet.presentationController.containerView.bounds) - ((CGFloat)wwidth.floatValue/2) , currentFrame.origin.y, (CGFloat)wwidth.floatValue, (CGFloat)wheight.floatValue);
                            
                        }else  if(href[@"windowheight"])
                        {
                            NSString *tmpwheight=href[@"windowheight"];
                            
                            return CGRectMake(CGRectGetMidX(weakFormSheet.presentationController.containerView.bounds) - (currentFrame.size.width/2) , currentFrame.origin.y, currentFrame.size.width, (CGFloat)tmpwheight.floatValue);
                            
                        }
                        else{
                            return CGRectMake(CGRectGetMidX(weakFormSheet.presentationController.containerView.bounds) - (currentFrame.size.width/2) , currentFrame.origin.y, currentFrame.size.width, currentFrame.size.height);
                        }
                    }
                    
                    return currentFrame;
                };
                [navigationController presentViewController:formSheetController animated:YES  completion:^{
                    //formSheetController.presentationController.contentViewSize = newNav.preferredContentSize;
                }
            ...
            if(!view || (view && [view.lowercaseString isEqualToString:@"jason"])){
               // Jason View
                ...
                if([transition isEqualToString:@"replace"]){
                    ...
                    MZFormSheetPresentationViewController *formSheetController = [[MZFormSheetPresentationViewController alloc] initWithContentViewController:tab];
                   // formSheetController.presentationController.contentViewSize = tab.preferredContentSize;
                    formSheetController.presentationController.shouldCenterVertically = YES;
                    ...
                    [root presentViewController:formSheetController animated:YES completion:^{
                        formSheetController.presentationController.contentViewSize = tab.view.bounds.size;
                    }
                ...
                ** Custom ViewControllers
                ...
                if([transition isEqualToString:@"modal"]){
                    ...
                    [self unlock];
                    MZFormSheetPresentationViewController *formSheetController = [[MZFormSheetPresentationViewController alloc] initWithContentViewController:nav];
                   // formSheetController.presentationController.contentViewSize = nav.preferredContentSize;//CGSizeMake(250, [UIScreen mainScreen].bounds.size.height );//CGSizeMake(250, 250); // or pass in UILayoutFittingCompressedSize to size automatically with auto-layout
                    formSheetController.presentationController.shouldCenterVertically = YES;
                    __weak typeof(formSheetController) weakFormSheet = formSheetController;
                    formSheetController.presentationController.frameConfigurationHandler = ^(UIView * __nonnull presentedView, CGRect currentFrame, BOOL isKeyboardVisible) {
                        if (UIInterfaceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
                    ...
                    [navigationController presentViewController:formSheetController animated:YES  completion:^{
                       // formSheetController.presentationController.contentViewSize = nav.preferredContentSize;
                    }
    ...
    @try{
        ...
        if(memory._stack && memory._stack.count > 0){
            ...
            ****************************************************/
            ...
            if(memory._stack[@"bind"]){
                NSDictionary * kv = memory._stack[@"bind"];
                //[[NSUserDefaults standardUserDefaults] setValue:kv[@"value"] forKey:kv[@"key"]];
                if(kv[@"bindclass"])
                {
                    [[[Globals sharedInstance] valueForKey:kv[@"bindclass"]] setValue:kv[@"value"] forKey:kv[@"key"]];
                }else{
                    [[[Globals sharedInstance] valueForKey:@"viewModel"] setValue:kv[@"value"] forKey:kv[@"key"]];
                }
