    ...
    for(NSDictionary *child in children){
        if(child && child.count > 0){
            ...
            if([type isEqualToString:@"vertical"] || [type isEqualToString:@"horizontal"]){
                ...
                }else if([component isKindOfClass:[UITextField class]]){
                   // if(form[@"txtrechargeamount"])
                  //  {
                    
                       // [[Jason client].model rz_bindKey:@"text" toKeyPath:@"txtrechargeamount" ofObject:form];
                       // [RACObserve(self, username) subscribeNext:^(NSString *newName) {
                          //  NSLog(@"%@", newName);
                       // }];
                   // }
                  /*  [[[NSUserDefaults standardUserDefaults] rac_valuesForKeyPath:@"txtrechargeamount" observer:(NSObject *)self] subscribeNext:^(id value) {
                       // [component se
                        [(UITextField *)component setText:value];
                    }];*/
                }
                if(isBuilding){
