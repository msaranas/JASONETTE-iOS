...
//
#import "JasonLynkapiAction.h"

@implementation JasonLynkapiAction
...
- (void)login{
    ...
    if(self.options){
      ...
      //  [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        ...
        /* u.UUID = @"162035877492930";
         
         u.email = @"msaravanakumar0@gmail.com";
         u.name = @"Saravanakumar";
         u.gender = @"male";
         u.login_type = @"google";
         u.contact=@"9840590739";
         u.Mobile=@"9840590739";
         u.thumb = @"https://scontent.fmaa1-2.fna.fbcdn.net/v/t1.0-9/13315262_1221737501170758_8565269636895900116_n.jpg?oh=a0cfd3841fa2e84b06721558ffe38900&oe=58FA0F82";
        ...
        u.UserName=@"9677151364";;//self.options[@"txtusername"];//@"8608506070";
        u.Password=@"12345678";//self.options[@"txtpassword"];//@"123456";
        if (u.UserName.length>0) {
            [ModelManager loginWithUser:u andToken:[Globals sharedInstance].deviceTokenString Status:YES withSuccess:^(NSString *u) {
               // [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
               // [self performSegueWithIdentifier:@"loginLynk" sender:self];
                [Util setBool:YES forKey:IS_LOGGED_IN_KEY];
                
                if(weakSelf.self.options[@"href"])
                {
                [[Jason client] go:weakSelf.self.options[@"href"]];
                }
    ...
    - (void)registeruser{
        ...
        if(self.options){
            ...
            [ModelManager registerAsCustomer:user withSuccess:^(NSDictionary *dic) {
                 [[Jason client] success: dic];
            } failure:^(NSString *err) {
                [[Jason client] error:err];
            }];
...
- (void)booknow{
    ...
    if(self.options){
        ...
        u.UserName=@"8608506070";;//self.options[@"txtusername"];//@"8608506070";
        u.Password=@"123456";//self.options[@"txtpassword"];//@"123456";
        if (u.UserName.length>0) {
            [ModelManager loginWithUser:u andToken:[Globals sharedInstance].deviceTokenString Status:YES withSuccess:^(NSString *u) {
                // [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
                // [self performSegueWithIdentifier:@"loginLynk" sender:self];
