...
- (void)viewWillLayoutSubviews
{
    ...
    if([navigationController isKindOfClass:[NavigationViewController class]] )//&& navigationController.menu !=nil)
    {
...
- (void)segmentedControlChangedValue:(HMSegmentedControl *)segmentedControl {
    ...
    if(button[@"action"]){
        [[Jason client] call:button[@"action"]];
    }
                        ...
                        if(body[@"type"]){
                            if([body[@"type"] isEqualToString:@"image"]){
                                ...
                                if(![url containsString:@"{{"] && ![url containsString:@"}}"]){
                                    ...
                                    [sdmanager loadImageWithURL:[NSURL URLWithString:url]
                                                          options:0
            ...
            if(chat_input[@"left"]){
                if(chat_input[@"left"][@"image"]){
                    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                        SDWebImageManager *sdmanager = [SDWebImageManager sharedManager];
                        [sdmanager loadImageWithURL:[NSURL URLWithString:chat_input[@"left"][@"image"]]
                                              options:0
                                             progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
